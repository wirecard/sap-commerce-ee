...
public class PaymentConfigurationPromotionFilter implements PaymentConfigurationFilter {
    ...
    public boolean isValid(WirecardPaymentConfigurationModel configuration, AbstractOrderModel order) {
        return restrictionDisabled(configuration) || getWirecardPaymentConfigurationService().hasNoDiscounts(order);
    }

    private boolean restrictionDisabled(WirecardPaymentConfigurationModel configuration) {
        return BooleanUtils.isNotTrue(configuration.getRestrictPromotions());
    }
