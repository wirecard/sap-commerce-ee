...
public class PaymentConfigurationShippingFilter implements PaymentConfigurationFilter {
    ...
    public boolean isValid(WirecardPaymentConfigurationModel configuration, AbstractOrderModel order) {
        ...
        if (configuration != null && order != null) {
            Collection<CountryModel> shippingCountries = configuration.getShippingCountries();
            if (CollectionUtils.isNotEmpty(shippingCountries)) {
                AddressModel deliveryAddress = order.getDeliveryAddress();
                if (deliveryAddress != null) {
                    CountryModel country = deliveryAddress.getCountry();
                    valid = shippingCountries.contains(country);
                }
            }
        }
