...
public class DefaultRefundResponseHandler extends AbstractResponseHandler<ReturnRequestModel> {
     ...
     */
    @Override
    public AbstractOrderModel getOrder(ReturnRequestModel model) {
     ...
     */
    @Override
    public BigDecimal getTransactionAmount(ReturnRequestModel item) {
        final BigDecimal customRefundAmount = refundAmountCalculationService.getCustomRefundAmount(item);
        ...
        if (customRefundAmount != null && customRefundAmount.compareTo(BigDecimal.ZERO) > 0) {
            ...
            amountToRefund = refundAmountCalculationService.getOriginalRefundAmount(item);
        }

        return amountToRefund.setScale(2, RoundingMode.HALF_EVEN);
    }
