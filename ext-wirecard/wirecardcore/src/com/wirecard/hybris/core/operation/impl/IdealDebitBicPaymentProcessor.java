...
public class IdealDebitBicPaymentProcessor implements PaymentProcessor {
    ...
    public void processPayment(Payment payment, PaymentOperationData data) throws WirecardPaymenException {
        ...
        payment.setOrderNumber(null);

        if (StringUtils.isNotEmpty(payment.getRequestId())) {
            StringBuilder successURL = new StringBuilder(payment.getSuccessRedirectUrl());
            successURL.append("?status=success&request_id=").append(payment.getRequestId());
            payment.setSuccessRedirectUrl(successURL.toString());
        }
