...
public class RatepayInvoiceFingerPrintPaymentProcessor implements PaymentProcessor {

    private ObjectFactory objectFactory;
    private PasswordEncoder genericEncoder;

    @Override
    public void processPayment(Payment payment, PaymentOperationData data) throws WirecardPaymenException {

        String encoded = genericEncoder.encode("uid", payment.getMerchantAccountId().getValue() +
            "_" + System.currentTimeMillis());

        Device device = objectFactory.createDevice();
        device.setFingerprint(encoded);
    ...
    protected PasswordEncoder getGenericEncoder() { return genericEncoder; }

    @Required
    public void setGenericEncoder(PasswordEncoder genericEncoder) { this.genericEncoder = genericEncoder; }
}
