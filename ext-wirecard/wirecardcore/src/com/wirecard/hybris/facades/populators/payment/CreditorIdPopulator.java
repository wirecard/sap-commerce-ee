...
public class CreditorIdPopulator extends AbstractOrderAwarePaymentPopulator {
    ...
    public void doPopulate(AbstractOrderModel source, Payment target) throws ConversionException {
        target.setCreditorId(getCreditroId(source));
    }

    private String getCreditroId(AbstractOrderModel source) {
        String creditorId = null;
        WirecardPaymentConfigurationModel wirecardPaymentConfiguration =
            getWirecardPaymentConfigurationService().getConfiguration(source.getStore(),
                                                                      source.getPaymentMode());
        if (wirecardPaymentConfiguration != null && wirecardPaymentConfiguration.getAuthentication() != null) {
            creditorId = wirecardPaymentConfiguration.getAuthentication().getCreditorId();
        }
        return creditorId;
    }
