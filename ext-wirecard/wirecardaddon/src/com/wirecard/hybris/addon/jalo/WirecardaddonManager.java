...
import com.wirecard.hybris.addon.constants.WirecardaddonConstants;

/**
 * This is the extension manager of the Wirecardaddon extension.
 */
public class WirecardaddonManager extends GeneratedWirecardaddonManager {

    /** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
    private static final Logger LOG = Logger.getLogger(WirecardaddonManager.class.getName());
     ...
     */
    public static WirecardaddonManager getInstance() {
        return (WirecardaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
                .getExtension(WirecardaddonConstants.EXTENSIONNAME);
    }
     ...
     */
    public WirecardaddonManager() // NOPMD
    {
        if (LOG.isDebugEnabled()) {
            LOG.debug("constructor of WirecardaddonManager called.");
        }
    }
    ...
    public void init() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("init() of WirecardaddonManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("destroy() of WirecardaddonManager called, current tenant: " + getTenant().getTenantID());
        }
    }
